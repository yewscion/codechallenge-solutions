(define-module (codechallenges minimum-number-of-jumps))
...
(define test-input '(1 3 5 8 9 2 6 7 6 8 9))

(display test-input)

(define (list->graph-list input)
  (cond ((= (length input) 0) '())
        (else (let ((item (car input))
                    (rest (cdr input)))
                (cons (jumpable-range item rest)
                      (list->graph-list rest))))))

(define (jumpable-range-values value rest)
  (cond ((< (length rest) value)
         (list value rest))
        (else
         (list value (list-head rest value)))))
